...
def call_recommender_llm(instruction: str, evals: dict, metric_names: list[str], optimizer: str, reco_format: str, top_points: list[dict], top_points_format: str, file_path: str, reco_llm_name: str, reco_temp: float = 1.0, reco_top_p: float = 0.95) -> str:
  """
  ...
  """
  recommender_prompt = create_recommender_prompt(instruction, evals, metric_names=metric_names, top_points=top_points, file_path=file_path, optimizer=optimizer, reco_format=reco_format, top_points_format=top_points_format)
...
if __name__ == '__main__':
  ...
  instruction = call_rater_llm_meta_prompt(rater_llm_name=rater_llm_name, top_k_prompts=top_k_prompts, rater_temp=0.0, rater_top_p=1.0, metric_names=metric_names)
  print(instruction)
  print('=' * 70)

  evals = call_rater_llm_prompt(instruction=instruction, file_path=filepath, num_examples=20, max_workers=10, rater_temp=0.0, rater_top_p=1.0, rater_llm_name=rater_llm_name, metric_names=metric_names)
  print(evals)
  ...
  top_points = find_most_informative_points(evals, top_k=5, metric_names=metric_names)
  print("Found most informative points")
  print(top_points)

  optimizer = 'min-all'
  reco_format = 'OPRO'
  recommendation = call_recommender_llm(instruction, metrics, reco_llm_name=reco_llm_name, top_points=top_points, file_path=filepath, metric_names=metric_names, optimizer=optimizer, reco_format=reco_format)
